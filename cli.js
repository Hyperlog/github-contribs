...
`,
  ...
  if (cli.flags.quiet && cli.flags.verbose) {
    console.error(
      'Error: --quiet and --verbose are mutually exclusive. See `github-contribs --help`.'
    );
    process.exit(1);
  }
  ...
  if (cli.input.length < 1) {
    console.error(
      'Error: USER argument missing. See `github-contribs --help`.'
    );
    process.exit(1);
  }
  ...
  if (cli.input.length > 1) {
    console.error(
      'Error: too many positional arguments. See `github-contribs --help`.'
    );
    process.exit(1);
  }
